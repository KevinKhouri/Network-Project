...
bool isAck(struct udp_pcb *pcb);
void sendAck(struct udp_pcb *pcb, int seqNum);
...
void rdp_recv(struct udp_pcb *pcb, udp_recv_fn recv, void *recvArg);
...
#define GET_THIS_OR_THIS_PARAMETER(X, Z, Y)           \
    if (((next_tok = strtok(NULL, " \n")) == NULL) || \
        ((strstr(next_tok, X) == NULL) &&             \
         (strstr(next_tok, Z) == NULL)))              \
    {                                                 \
        error(Y);                                     \
        return;                                       \
    }
...
///////////////////// STUDENT CODE BELOW FOR ASSIGNMENT 1 //////////////////////////////////

//Used for sending side.
int nextSeqNum = 0;
//Used for receiving side.
int expectedSeqNum = 0;
udp_recv_fn applicationCallback;

//first bit in 2-bit flags is Ack, Second bit is Seq Num.

bool isAck(struct udp_pcb *pcb)
{
    return (pcb->rdpFlags >= 2);
}

void sendAck(struct udp_pcb *pcb, int seqNum)
{
    pcb->local_port = (((2 | seqNum) & 0x3) << 14) | pcb->local_port;
    char payload[DEFAULT_MTU];
    // create pbuf and call udp_send()
    struct pbuf *p = pbuf_alloc(PBUF_TRANSPORT, strlen(payload), PBUF_RAM);
    p->payload = payload;
...
err_t rdp_send(struct udp_pcb *pcb, struct pbuf *p)
{
    ...
    double timeout = 3000; //1 second for timeout

    int currentSeqNum = nextSeqNum;

retransmit:
    pcb->local_port = ((currentSeqNum & 0x3) << 14) | pcb->local_port;
    pcb->rdpFlags = 0;
    udp_send(pcb, p);
    uint64_t timer = 0;
    gettimeofday(&start, NULL);
    while (timer <= timeout && nextSeqNum == currentSeqNum)
    {
        gettimeofday(&stop, NULL);
        timer = (stop.tv_sec - start.tv_sec) * 1000 + (stop.tv_usec - start.tv_usec) / 1000;
    }
    if (nextSeqNum == currentSeqNum)
        goto retransmit;
...
void rdp_recv_callback(void *arg, struct udp_pcb *pcb, struct pbuf *p, uchar *addr, uint16_t port)
{
    ...
    {
        int seqNumToAck = expectedSeqNum;
        if ((pcb->rdpFlags & 1) == expectedSeqNum)
        {
            expectedSeqNum = (expectedSeqNum + 1) % 2;
            applicationCallback(arg, pcb, p, addr, port);
            sendAck(pcb, seqNumToAck);
        }
        else
        {
            sendAck(pcb, (seqNumToAck + 1) % 2);
        }
        pcb->rdpFlags = 0;
        pbuf_free(p);
    }
    ...
    pseudocode:
     if (isAck(p)){
        if (packets sequence number == nextSeqNum) {
            stopTimer; //this is likely done by just changing the nextSeqNum.
            nextSeqNum++;
        } else { //received duplicated ack
            //do nothing
        }
    } else { //packet is not an ACK, so must be new data
        if (packet's sequence number == expectedSeqNum) {
            call applicationCallBack
            sendAck(with expectedSeqNum);
            expectedSeqNum++;
        } else {
            sendAck(with (expectedSeqNum+1)%2);
        }
    }
    */
}
...
void rdp_recv(struct udp_pcb *pcb, udp_recv_fn recv, void *recvArg)
{
    applicationCallback = recv;
    udp_recv(pcb, rdp_recv_callback, pcb);
}

//////////////////// STUDENT CODE ABOVE FOR ASSIGNMENT 1  //////////////////////////////////
